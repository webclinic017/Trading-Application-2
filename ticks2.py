    ...
    try:
        ...
        if len(ohlc_final_1min) > 0:  # checking if there is atleast 1 candle in OHLC Dataframe
            if ohlc_final_1min.iloc[-1, 7] != 0:  # checking that we do not have the ATR value as 0
                ...
                ########################################################
                if (company_data['last_price'] >= ohlc_final_1min.iloc[-1, 7] + RENKO[company_data['instrument_token']][
                    1]) and RENKO[company_data['instrument_token']][3] == "Signal":
                ...
                elif (company_data['last_price'] >= ohlc_final_1min.iloc[-1, 7] +
                      RENKO[company_data['instrument_token']][1]) and RENKO[company_data['instrument_token']][
                    3] == "BUY":
                    RENKO[company_data['instrument_token']][2] = RENKO[company_data['instrument_token']][1] + \
                                                                 ohlc_final_1min.iloc[-1, 7]
                    RENKO[company_data['instrument_token']][3] = "BUY"
                ...
                elif (company_data['last_price'] <= RENKO[company_data['instrument_token']][1] - ohlc_final_1min.iloc[
                    -1, 7]) and RENKO[company_data['instrument_token']][3] == "Signal":
                    RENKO[company_data['instrument_token']][2] = RENKO[company_data['instrument_token']][1] - \
                                                                 ohlc_final_1min.iloc[-1, 7]
                    ...
                    -1, 7]) and RENKO[company_data['instrument_token']][3] == "SELL":
                    RENKO[company_data['instrument_token']][2] = RENKO[company_data['instrument_token']][1] - \
                                                                 ohlc_final_1min.iloc[-1, 7]
                    RENKO[company_data['instrument_token']][3] = "SELL"
                    RENKO_temp = pd.DataFrame([RENKO[x]], columns=["Symbol", "Open", "Close", "Signal", "Position"])
                ...
                if len(RENKO_Final) > 0:
                    if RENKO_Final.iloc[-1, 3] == "BUY" and company_data['last_price'] <= \
                    ...
                    elif RENKO[company_data['instrument_token']][3] == "SELL" and company_data['last_price'] >= \
                            RENKO[company_data['instrument_token']][1] + (
...
def calcpsoitions(Token, quantity, Last_price, Signal):
    ...
    if profit[Token][1] != 0 and profit[Token][2] != 0:
        ...
        profit[Token][3] = ((profit[Token][1] - profit[Token][2]) * quantity) - (
                    BuyBrokerage + SellBrokerage + STT + TNXChrgs + GST + SEBIChrgs + StampDuty)
...
def on_ticks(ws, ticks):  # retrive continius ticks in JSON format
    ...
    try:
        for company_data in ticks:
            ...
            if len(RENKO_Final) > 0:
                if ohlc_final_1min.iloc[-1, 5] < RENKO[company_data['instrument_token']][2] and RENKO_Final.iloc[
                    -1, 3] == "SELL" and RENKO[company_data['instrument_token']][4] != "SHORT":
                ...
                elif ohlc_final_1min.iloc[-1, 5] > RENKO[company_data['instrument_token']][2] and RENKO_Final.iloc[
                    -1, 3] == "BUY" and RENKO[company_data['instrument_token']][4] != "LONG":
