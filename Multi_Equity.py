...
class Trading:
    ...
    access_token = "af1TLBPKAKQoc8LGjIEIfXh7vprHxUfI"
    kws = KiteTicker(api_k, access_token)
    kite = KiteConnect(api_key=api_k, access_token=access_token)
    n = 14
    Position = ''

    def __init__(self, ins_code, symbol):
        self.ins_code = ins_code
        self.symbol = symbol

    with urlopen("https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=NSE:"+ self.symbol +"&interval=1min&outputsize=full&apikey=08AXPZ0UQEVKUSD8") as response:
    ...
    def target(Orderid, direction):
        global ohlc_temp
        Order_data = Trading.kite.order_history(Orderid)
    ...
    def quantity(ltp):
        ...
        mar = KiteConnect.margins(Trading.kite)
    ...
    ohlc_final_1min = pd.DataFrame(
        columns=["Symbol", "Time", "Open", "High", "Low", "Close", "TR", "ATR", "SMA", "TMA"])
    ...
    ohlc = ["Symbol", "Time", 0, 0, 0, 0, 0, 0, 0,0]  # [Symbol, Traded Time, Open, High, Low, Close, True Range, Average True Range, Simple Moving Average, Triangular moving average]
    RENKO = ["Symbol", 0, 0, "Signal", "None", 0, 0]
    profit = ["Symbol", 0, 0, "Profit"]
        ...
        try:
            ...
            if (str(((company_data["timestamp"]).replace(second=0))) != ohlc[company_data['instrument_token']][1]) and (
                ...
                if len(ohlc_final_1min) < 13:
                    ...
                    a = [ohlc_temp.iloc[-1, 6], ohlc_final_1min.iloc[-1, 6], ohlc_final_1min.iloc[-2, 6],
                         ohlc_final_1min.iloc[-3, 6], ohlc_final_1min.iloc[-4, 6],
                         ohlc_final_1min.iloc[-5, 6], ohlc_final_1min.iloc[-6, 6], ohlc_final_1min.iloc[-7, 6],
                         ohlc_final_1min.iloc[-8, 6], ohlc_final_1min.iloc[-9, 6],
                ...
                if len(ohlc_final_1min) < 10:
                    ...
                    b = [ohlc_temp.iloc[-1, 5], ohlc_final_1min.iloc[-1, 5], ohlc_final_1min.iloc[-2, 5],
                         ohlc_final_1min.iloc[-3, 5], ohlc_final_1min.iloc[-4, 5],
                         ohlc_final_1min.iloc[-5, 5], ohlc_final_1min.iloc[-6, 5], ohlc_final_1min.iloc[-7, 5],
                         ohlc_final_1min.iloc[-8, 5], ohlc_final_1min.iloc[-9, 5]]
                ...
                if len(ohlc_final_1min) < 19:
                    ...
                    c = [ohlc_temp.iloc[-1, 8], ohlc_final_1min.iloc[-1, 8], ohlc_final_1min.iloc[-2, 8],
                         ohlc_final_1min.iloc[-3, 8], ohlc_final_1min.iloc[-4, 8],
                         ohlc_final_1min.iloc[-5, 8], ohlc_final_1min.iloc[-6, 8], ohlc_final_1min.iloc[-7, 8],
                         ohlc_final_1min.iloc[-8, 8], ohlc_final_1min.iloc[-9, 8]]
                    ...
                    if RENKO[company_data['instrument_token']][3] == "BUY":
                        if (company_data['last_price'] >= ohlc_final_1min.iloc[-1, 7] +
                            ...
                            RENKO[company_data['instrument_token']][1] = RENKO_Final.iloc[-1, 2]
                                ...
                                RENKO_Final.iloc[-1, 2] - RENKO_Final.iloc[-1, 1]) - ohlc_final_1min.iloc[-1, 7]:
                            RENKO[company_data['instrument_token']][1] = RENKO_Final.iloc[-1, 1]
                            RENKO[company_data['instrument_token']][2] = RENKO_Final.iloc[-1, 1] - ohlc_final_1min.iloc[
                                -1, 7]
                            RENKO[company_data['instrument_token']][3] = "SELL"
                            RENKO_temp = pd.DataFrame([RENKO[x]],
                                                      columns=["Symbol", "Open", "Close", "Signal", "Position", "SMA",
                    ...
                    if RENKO[company_data['instrument_token']][3] == "SELL":
                        if (company_data['last_price'] <= RENKO[company_data['instrument_token']][1] -
                            ...
                            RENKO[company_data['instrument_token']][1] = RENKO_Final.iloc[-1, 2]
                                ...
                                RENKO_Final.iloc[-1, 1] - RENKO_Final.iloc[-1, 2]) + ohlc_final_1min.iloc[-1, 7]:
                            RENKO[company_data['instrument_token']][1] = RENKO_Final.iloc[-1, 1]
                            RENKO[company_data['instrument_token']][2] = RENKO_Final.iloc[-1, 1] + ohlc_final_1min.iloc[
                                -1, 7]
                            RENKO[company_data['instrument_token']][3] = "BUY"
        ...
        try:
            if len(RENKO_Final) > 0:
                if RENKO_Final.iloc[-1, 6] != 0:
                    if ((RENKO_Final.iloc[-1, 3] == "SELL") & ((RENKO_Final.iloc[-1, 1] < RENKO_Final.iloc[-1, 6]) & (
                            RENKO_Final.iloc[-1, 2] < RENKO_Final.iloc[-1, 6]))):
                        if Direction != "Down":
                            if (positions(company_data['instrument_token']) == 0):
                                Orderid = self.place_order(variety="regular", exchange=self.EXCHANGE_NSE,
                                                           tradingsymbol=trd_portfolio[
                            ...
                            if (positions(company_data['instrument_token']) > 0):
                                self.modify_order(variety="regular", order_id=Target_order_id,
                                                  order_type=self.ORDER_TYPE_MARKET)
                    elif ((RENKO_Final.iloc[-1, 3] == "BUY") & ((RENKO_Final.iloc[-1, 1] > RENKO_Final.iloc[-1, 6]) & (
                            RENKO_Final.iloc[-1, 2] > RENKO_Final.iloc[-1, 6]))):
                        if Direction != "Up":
                            if (positions(company_data['instrument_token']) == 0):
                                Orderid = self.place_order(variety="regular", exchange=self.EXCHANGE_NSE,
                    ...
                    if (positions(company_data['instrument_token']) > 0):
                        if Target_order != "YES":
                            Target_order_id = self.place_order(variety="regular", exchange=self.EXCHANGE_NSE,
                                                               tradingsymbol=trd_portfolio[
                                                                   company_data['instrument_token']],
                                                               transaction_type=self.TRANSACTION_TYPE_SELL,
                                                               quantity=abs(
                                                               ...
                                                               price=round(target(Orderid, Direction), 1),
                    ...
                    if ((positions(company_data['instrument_token'])) < 0):
                        if Target_order != "YES":
                            Target_order_id = self.place_order(variety="regular", exchange=self.EXCHANGE_NSE,
                                                               tradingsymbol=trd_portfolio[
                                                                   company_data['instrument_token']],
                                                               transaction_type=self.TRANSACTION_TYPE_BUY, quantity=abs(
                                    positions(company_data['instrument_token'])),
                                                               order_type=self.ORDER_TYPE_LIMIT,
                                                               price=round(target(Orderid, Direction), 1),
                                                               product=self.PRODUCT_MIS)
    ...
    def on_ticks(ws, ticks):  # retrieve continuous ticks in JSON format
        ...
        try:
            for company_data in ticks:
                ...
                company_data['last_trade_time'].time()) < datetime.time(15, 31, 00):
