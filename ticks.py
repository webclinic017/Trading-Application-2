...
access_token = "QmZO7lywjj91KPUs4oNOnKRQ9M2jwgPX"
...
def on_ticks(ws, ticks):  # retrieve continuous ticks in JSON format
    ...
    try:
        for company_data in ticks:
            Orderid = self.order_history(190802001525319)
            print(Orderid)
            for item in Orderid:
                if item['status'] == "COMPLETE":
                    print(item['average_price'])
            #print(next(item['average_price'] for item in Orderid if item['status'] == "COMPLETE"))
